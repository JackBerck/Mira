...
export default function MyCollaborations({
    ...
    const handleDelete = (collaborationId: number, title: string) => {
        if (
            confirm(
                `Yakin ingin menghapus kolaborasi "${title}"? Tindakan ini tidak dapat dibatalkan.`,
            )
        ) {
            router.delete(`/beranda/kolaborasi/${collaborationId}`, {
                onSuccess: () => {
